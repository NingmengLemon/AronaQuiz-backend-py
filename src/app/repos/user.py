...
class UserRepository(BaseRepository[DBUser]):
    ...
    async def get_by_username(self, session: AsyncSession, username: str) -> DBUser | None:
        """根据用户名获取用户"""
    ...
    async def get_by_user_id(self, session: AsyncSession, user_id: UUID) -> DBUser | None:
        """根据用户ID获取用户"""
